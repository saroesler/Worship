...
class Worship_Controller_Admin extends Zikula_AbstractController
{
    ...
    public function viewmanage()
    {
    	...
    	{
    	case 'all':
    		$this->redirect(ModUtil::url($this->name, 'user', 'all'));
    	case 'special':
    		$this->redirect(ModUtil::url($this->name, 'user', 'special'));
    	case 'daily':
    		$this->redirect(ModUtil::url($this->name, 'user', 'daily'));
    	}
    }
    ...
    public function ChurchAdd()
    {
    	...
    	case 'add':
			...
			//Special-worship for this church deleate:
			...
			//del alle daily worships for this church
			//db of all daily worships for this church
			$dai_worships = $this->entityManager->getRepository('Worship_Entity_Churchesworships')->findBy(array('cid' => $actionid));
			foreach($dai_worships as $dai_worship)//for every worship
			{
				//get worship ID
				$cwid = $dai_worship->getcwid();
				//del worship
				$worship = $this->entityManager->find('Worship_Entity_Churchesworships', $cwid);
				$this->entityManager->remove($worship);
				$this->entityManager->flush();
			}
			//del church
    ...
    *******************/
    ...
    public function specialdayManage()
    {
		...
		case 'del':
			...
			//Special-worship for this day deleate:
			//db of all special worships for this day
			$spe_worships = $this->entityManager->getRepository('Worship_Entity_Worships')->findBy(array('did' => $actionid));
			foreach($spe_worships as $spe_worship)//for every specialworship
			{
				//get worship ID
				$Wid = $spe_worship->getWid();
